...
- JDBC为访问不同的数据库提供了一种统一的途径，为开发者屏液了一些细节问题。
- JDBC的目标是使Java程序员使JDBC可以连接任何提供了JDBC驱动程序的数据库系统，这样就使得程序员无需对特定的数据库系统的特点有过多的了解，从而大大简化和加快了开发过程。
...
>- 通常下用显式调用DriverManager 类的 registerDriver() 方法来注册驱动程序类的实例，因为Driver接口的驱动程序类都包含了静态代码块，在这个静态代码块中，会调用DriverManager.registerDriver() 方法来注册自身的一个实例
...
public void test5() throws Exception {
    ...
    //1.获取关联配置文件的输入流
    final InputStream in = ConnectionTest.class.getClassLoader().getResourceAsStream("jdbc.properties");
    if (in == null){
...
public void test6() throws ClassNotFoundException, SQLException {
    ...
    Class.forName("com.mysql.jdbc.Driver");
    //2.获取连接
    final Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/db3"
                                                              , "root", "root");
    System.out.println(connection);
}
```
